...
const getClient = async (req, res) => {
  try {
    ...
    const endpoints = [
      { url: config.endpoint_cliente_bd, token: config.token_bd },
      { url: config.endpoint_cliente_cn, token: config.token_cn },
      { url: config.endpoint_cliente_364, token: config.token_364 },
    ];
    ...
    const filteredData = validResults.map((result) => ({
      fulldata: result.data.registros,
      idCliente: result.data.registros[0].id,
      nomeCliente: result.data.registros[0].razao,
